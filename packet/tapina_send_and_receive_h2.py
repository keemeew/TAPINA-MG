...
def send_and_receive():
    ...
    send1_thread = threading.Thread(target=send_packet1, args=())
    send1_thread.daemon = True    
    send1_thread.start()

    send2_thread = threading.Thread(target=send_packet2, args=())
    send2_thread.daemon = True    
    send2_thread.start()
    ...
    send4_thread = threading.Thread(target=send_packet4, args=())
    send4_thread.daemon = True    
    send4_thread.start()

    send5_thread = threading.Thread(target=send_packet5, args=())
    send5_thread.daemon = True    
    send5_thread.start()

    num_packets = args.packet_number
    # generate_packets(num_packets, job_id, src_mac, dst_mac, src_ip)
    pkts1 = generate_packets(num_packets, 1, "00:00:0a:00:01:02", "00:01:0a:00:01:02", "10.0.1.2")  ## need to modify for each host
    pkts2 = generate_packets(num_packets, 2, "00:00:0a:00:01:02", "00:01:0a:00:01:02", "10.0.1.2")  ## need to modify for each host
    pkts3 = generate_packets(num_packets, 3, "00:00:0a:00:01:02", "00:01:0a:00:01:02", "10.0.1.2")  ## need to modify for each host
    pkts4 = generate_packets(num_packets, 4, "00:00:0a:00:01:02", "00:01:0a:00:01:02", "10.0.1.2")  ## need to modify for each host
    pkts5 = generate_packets(num_packets, 5, "00:00:0a:00:01:02", "00:01:0a:00:01:02", "10.0.1.2")  ## need to modify for each host
...
if __name__ == '__main__':
    ...
    sys.stdout = Logger("/home/tofino/TAPINA/results/h2.txt")
    ...
    iface = "h2-eth0" ## need to modify for each host
